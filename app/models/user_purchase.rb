class UserPurchase
  ...
  attr_accessor :postal_code, :delivery_prefecture_id, :city, :house_number, :building_name, :phone_number, :item_id, :user_id
  # , :token　　　　payjpで解除

  # validates :token　　　　payjpで解除
  with_options presence: true do
    validates :postal_code, format: {with: /\A[0-9]{3}-[0-9]{4}\z/, message: "is invalid. Include hyphen(-)"}
    validates :city, format: { with: /\A[ぁ-んァ-ン一-龥]/, message: "is invalid. Input full-width characters."}
    validates :house_number
    validates :phone_number, format: {with: /0\d{9,10}\z/, message: "is invalid. Input half-width characters."}
  end
